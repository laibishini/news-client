...
<div class="content">
    ...
    <div class="cont">
        ...
        <div class="text-box" v-html="arrlist.content">
...
<script>
	export default {
		mounted(){
				// this.fachData(this.$route.params.id);
		},
  ...
  watch:{
  		$route(to){
  			...
  			var reg=/acticle\/\d+/;
      if(reg.test(to.path)){
      	var article = parseInt(this.$route.params.id)-1;
      		// article === 1 ? 0 : article
      		console.log(article)
    ...
    fachData(id){
      var that = this
      this.$http.get('../src/data/article.data').then(function(result){
         	console.log(result.data[0])
          that.arrlist = result.data[id]
           console.log(that.arrlist)
      }).catch(function(err){
      	console.log("内容错误",err)
      })
    }
  }
